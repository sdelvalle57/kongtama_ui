...
export class BaseStepModal extends React.Component<Props, State> {
  ...
  public render = () => {
    ...
    const { loadingStarted, status, message } = this.state
    const retry = () => this._retry()
    let content: ReactNode
    let bodyText: ReactNode
    let footer = this.props.showPartialProgress ? null : (
      <ModalStatusTextLight>{}</ModalStatusTextLight>
    )
    switch (status) {
      ...
      case StepStatus.Done:
        ...
        footer = (
          <ModalStatusTextLight>{doneFooterCaption}</ModalStatusTextLight>
        )
        break
      case StepStatus.Error:
        content = <StepStatusError message={message} />
        bodyText = (
          <ModalText>
            {errorCaption}
            <br />
            <span className="modal-text-clickable" onClick={retry}>
      ...
      default:
        ...
        footer = (
          <ModalStatusTextLight>{loadingFooterCaption}</ModalStatusTextLight>
        )
        break
    }
